...
interface WrapperStudioProps {
  image: string
  title: string
}

export default function WrapperStudio ({
  image, title
}: WrapperStudioProps) {
  ...
  return <Link href={'/estudio'} ref={refStudio} className={`${inViewStudio ? 'animatecss animatecss-fast animatecss-slideInLeft' : 'opacity-0'} relative transition duration-[0.8s] hover:scale-[1.02] w-[100%] h-[100%]`}>
  <picture>
